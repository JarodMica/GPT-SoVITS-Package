...
cache= {}
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut=i18n("不切"), top_k=20, top_p=0.6, temperature=0.6, ref_free = False,speed=1,if_freeze=False,inp_refs=None,sample_steps=8,if_sr=False,pause_second=0.3, seed_toggle=False, seed_number=-1):
    global cache
    if seed_toggle:
    ...
    with gr.Group():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=7):
                ...
                seed_toggle = gr.Checkbox(value=False, label="Set Seed")
                seed_number = gr.Textbox(label="Seed Value", visible=False)
            # with gr.Column():
        ...
        inference_button.click(
            get_tts_wav,
            [inp_ref, prompt_text, prompt_language, text, text_language, how_to_cut, top_k, top_p, temperature, ref_text_free,speed,if_freeze,inp_refs,sample_steps,if_sr_Checkbox,pause_second_slider, seed_toggle, seed_number],
            [output],
        )
        ...
        seed_toggle.change(toggle_on_seed, [seed_toggle],[seed_number])
