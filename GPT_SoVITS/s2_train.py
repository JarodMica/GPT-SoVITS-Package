...
def run(rank, n_gpus, hps):
    ...
    try:  # 如果能加载自动resume
        _, _, _, epoch_str = GPT_SoVITS.utils.load_checkpoint(
            GPT_SoVITS.utils.latest_checkpoint_path("%s/logs_s2_%s" % (hps.data.exp_dir,hps.model.version), "D_*.pth"),
            net_d,
            optim_d,
        )  # D多半加载没事
        if rank == 0:
        ...
        _, _, _, epoch_str = GPT_SoVITS.utils.load_checkpoint(
            GPT_SoVITS.utils.latest_checkpoint_path("%s/logs_s2_%s" % (hps.data.exp_dir,hps.model.version), "G_*.pth"),
            net_g,
            optim_g,
        )
        epoch_str+=1
