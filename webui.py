...
def change_label(path_list):
    ...
    if p_label is None:
        ...
        cmd = '"%s" GPT_SoVITS/tools/subfix_webui.py --load_list "%s" --webui_port %s --is_share %s'%(python_exec,path_list,webui_port_subfix,is_share)
...
def change_uvr5():
    global p_uvr5
    if p_uvr5 is None:
        cmd = '"%s" GPT_SoVITS/tools/uvr5/webui.py "%s" %s %s %s'%(python_exec,infer_device,is_half,webui_port_uvr5,is_share)
...
def open_asr(asr_inp_dir, asr_opt_dir, asr_model, asr_model_size, asr_lang, asr_precision):
    ...
    if p_asr is None:
        ...
        cmd = f'"{python_exec}" GPT_SoVITS/tools/asr/{asr_dict[asr_model]["path"]}'
        cmd += f' -i "{asr_inp_dir}"'
        cmd += f' -o "{asr_opt_dir}"'
...
def open_denoise(denoise_inp_dir, denoise_opt_dir):
    ...
    if(p_denoise==None):
        ...
        cmd = '"%s" GPT_SoVITS/cmd-denoise.py -i "%s" -o "%s" -p %s'%(python_exec,denoise_inp_dir,denoise_opt_dir,"float16"if is_half==True else "float32")
...
def open1Ba(batch_size,total_epoch,exp_name,text_low_lr_rate,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers1Ba,pretrained_s2G,pretrained_s2D,if_grad_ckpt,lora_rank,if_test_all_lora):
    global p_train_SoVITS
    lora_ranks = ["16", "32", "64", "128"]
    if not p_train_SoVITS and if_test_all_lora:
        for lora_rank in lora_ranks:
            print(f"Preparing lora rank {lora_rank} for training")
...
def open_slice(inp,opt_root,threshold,min_length,min_interval,hop_size,max_sil_kept,_max,alpha,n_parts):
    ...
    if (ps_slice == []):
        for i_part in range(n_parts):
            cmd = '"%s" GPT_SoVITS/tools/slice_audio.py "%s" "%s" %s %s %s %s %s %s %s %s %s''' % (python_exec,inp, opt_root, threshold, min_length, min_interval, hop_size, max_sil_kept, _max, alpha, i_part, n_parts)
            print(cmd)
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem(i18n("1-GPT-SoVITS-TTS")):
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        with gr.Column():
                            ...
                            if_test_all_lora = gr.Checkbox(label="Test all LoRAs", value=False, interactive=True, show_label=True)
            ...
            button1Ba_open.click(open1Ba, [batch_size,total_epoch,exp_name,text_low_lr_rate,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers1Ba,pretrained_s2G,pretrained_s2D,if_grad_ckpt,lora_rank, if_test_all_lora], [info1Ba,button1Ba_open,button1Ba_close])
